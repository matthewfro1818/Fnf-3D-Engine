...
class PlayState3D extends MusicBeatState
{
	...
	public var notes:Array<Note3D> = [];
	public var unspawnNotes:Array<Note3D> = [];
	...
	public var strumLineNotes:Array<StrumNote3D> = [];
	public var opponentStrums:Array<StrumNote3D> = [];
	public var playerStrums:Array<StrumNote3D>   = [];
	...
	public static var ps3DInst:PlayState3D = null;
	...
	public var lTex:TextureMaterial;
	public var dTex:TextureMaterial;
	public var uTex:TextureMaterial;
	public var rTex:TextureMaterial;

	public var lsTex:TextureMaterial;
	public var dsTex:TextureMaterial;
	public var usTex:TextureMaterial;
	public var rsTex:TextureMaterial;

	public var lcTex:TextureMaterial;
	public var dcTex:TextureMaterial;
	public var ucTex:TextureMaterial;
	public var rcTex:TextureMaterial;

	public var lhTex:TextureMaterial;
	public var dhTex:TextureMaterial;
	public var uhTex:TextureMaterial;
	public var rhTex:TextureMaterial;

	public var leTex:TextureMaterial;
	public var deTex:TextureMaterial;
	public var ueTex:TextureMaterial;
	public var reTex:TextureMaterial;
	...
	override public function create()
	{
		...
		PlayState.isPixelStage = isPixelStage;
		PlayState.SONG = SONG;
		PlayState.isStoryMode = isStoryMode;
		PlayState.storyWeek = storyWeek;
		PlayState.storyPlaylist = storyPlaylist;
		PlayState.storyDifficulty = storyDifficulty;
		PlayState.practiceMode = practiceMode;
		PlayState.usedPractice = usedPractice;
		PlayState.changedDifficulty = changedDifficulty;
		PlayState.cpuControlled = cpuControlled;
		PlayState.campaignScore = campaignScore;
		PlayState.campaignMisses = campaignMisses;
		PlayState.seenCutscene = seenCutscene;
		PlayState.deathCounter = deathCounter;
		...
		// still. Better than loading a texture material for every note. Even if the code looks a little worse.
		lTex = new TextureMaterial(Cast.bitmapTexture("assets/models/leftn.png"));
		dTex = new TextureMaterial(Cast.bitmapTexture("assets/models/downn.png"));
		uTex = new TextureMaterial(Cast.bitmapTexture("assets/models/upn.png"));
		rTex = new TextureMaterial(Cast.bitmapTexture("assets/models/rightn.png"));
		
		lsTex = new TextureMaterial(Cast.bitmapTexture("assets/models/staticleft.png"));
		dsTex = new TextureMaterial(Cast.bitmapTexture("assets/models/staticdown.png"));
		usTex = new TextureMaterial(Cast.bitmapTexture("assets/models/staticup.png"));
		rsTex = new TextureMaterial(Cast.bitmapTexture("assets/models/staticright.png"));

		lcTex = new TextureMaterial(Cast.bitmapTexture("assets/models/confirmleft.png"));
		dcTex = new TextureMaterial(Cast.bitmapTexture("assets/models/confirmdown.png"));
		ucTex = new TextureMaterial(Cast.bitmapTexture("assets/models/confirmup.png"));
		rcTex = new TextureMaterial(Cast.bitmapTexture("assets/models/confirmright.png"));
		
		lhTex = new TextureMaterial(Cast.bitmapTexture("assets/models/lefth.png"));
		dhTex = new TextureMaterial(Cast.bitmapTexture("assets/models/downh.png"));
		uhTex = new TextureMaterial(Cast.bitmapTexture("assets/models/uph.png"));
		rhTex = new TextureMaterial(Cast.bitmapTexture("assets/models/righth.png"));

		leTex = new TextureMaterial(Cast.bitmapTexture("assets/models/lefte.png"));
		deTex = new TextureMaterial(Cast.bitmapTexture("assets/models/downe.png"));
		ueTex = new TextureMaterial(Cast.bitmapTexture("assets/models/upe.png"));
		reTex = new TextureMaterial(Cast.bitmapTexture("assets/models/righte.png"));

		lTex.alphaBlending = true; lsTex.alphaBlending = true; lcTex.alphaBlending = true;
		dTex.alphaBlending = true; dsTex.alphaBlending = true;
		uTex.alphaBlending = true; usTex.alphaBlending = true; dcTex.alphaBlending = true;
		rTex.alphaBlending = true; rsTex.alphaBlending = true;
		lhTex.alphaBlending = true; leTex.alphaBlending = true; ucTex.alphaBlending = true;
		dhTex.alphaBlending = true; deTex.alphaBlending = true;
		uhTex.alphaBlending = true; ueTex.alphaBlending = true; rcTex.alphaBlending = true;
		rhTex.alphaBlending = true; reTex.alphaBlending = true;
		...
		curStage = PlayState3D.SONG.stage;
		trace('stage is: ' + curStage);
		if(PlayState3D.SONG.stage == null || PlayState3D.SONG.stage.length < 1) {
			switch (songName)
			{
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			leftCnter = (opponentStrums[0].x + opponentStrums[1].x + opponentStrums[2].x + opponentStrums[3].x) / 4;
			rigtCnter = (playerStrums[0].x + playerStrums[1].x + playerStrums[2].x + playerStrums[3].x) / 4;
	...
	function startSong():Void
	{
		...
		FlxG.sound.playMusic(Paths.inst(PlayState3D.SONG.song), 1, false);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices)
			vocals = new FlxSound().loadEmbedded(Paths.voices(PlayState3D.SONG.song));
		else
			vocals = new FlxSound();

		FlxG.sound.list.add(vocals);
		FlxG.sound.list.add(new FlxSound().loadEmbedded(Paths.inst(PlayState3D.SONG.song)));

		//notes = new FlxTypedGroup<Note3D>();
		//add(notes);
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				if(songNotes[1] > -1) { //Real notes
					...
					var swagNote:Note3D = new Note3D(daStrumTime + ClientPrefs.noteOffset, daNoteData, [lTex, dTex, uTex, rTex][daNoteData % 4], false);
					swagNote.mustPress = gottaHitNote;
					unspawnNotes.push(swagNote);

					swagNote.x = 92 + ((daNoteData * Note.swagWidth) + (gottaHitNote ? FlxG.width / 2 : 0));
					/*swagNote.sustainLength = songNotes[2];
					swagNote.noteType = songNotes[3];
					if(!Std.isOfType(songNotes[3], String)) swagNote.noteType = editors.ChartingState.noteTypeList[songNotes[3]]; //Backward compatibility + compatibility with Week 7 charts
					swagNote.scrollFactor.set();*/

					var susLength:Int = Math.floor(songNotes[2] / Conductor.stepCrochet);

					//susLength = susLength / Conductor.stepCrochet;
					//unspawnNotes.push(swagNote);

					if(susLength > 0) for(i in 1...susLength+1){
						var textureArray = [lhTex, dhTex, uhTex, rhTex];
						if(i == susLength) textureArray = [leTex, deTex, ueTex, reTex];

						var susNote:Note3D = new Note3D(daStrumTime + (Conductor.stepCrochet * i), daNoteData, textureArray[daNoteData % 4], true);
						susNote.mustPress = gottaHitNote;
						susNote.isHoldEnd = true;
						...
						susNote.x = 92 + ((daNoteData * Note.swagWidth) + (gottaHitNote ? FlxG.width / 2 : 0));
					}
	...
	public var leftCnter:Float = 0;
	public var rigtCnter:Float = 0;
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		{
			// FlxG.log.add(i);
			var babyArrow:StrumNote3D = new StrumNote3D(i, player, [lsTex, dsTex, usTex, rsTex][i]);
			babyArrow.x = 92 + ((i * Note.swagWidth) + ((FlxG.width / 2) * player));

			//babyArrow.alpha = 0;
			//FlxTween.tween(babyArrow, {y: startY + 10}, 1, {ease: FlxEase.circOut, startDelay: 0.5 + (0.2 * i)});
			...
			babyArrow.defX = babyArrow.x;
			babyArrow.defY = babyArrow.y;
			babyArrow.defZ = babyArrow.z;
		}
	}
	...
	override public function update(elapsed:Float)
	{
		...
		//{
		//	for(daNote in notes) daNote.yaw = 45;
		//}

		
		/*for(dumStrum in strumLineNotes){
			var angle = Math.sin(((Conductor.songPosition + (Conductor.stepCrochet * dumStrum.ID)) / (Conductor.crochet * 2)) * Math.PI) * 20;
			dumStrum.pitch = angle;
			dumStrum.y = strumLine.y + (Math.sin((angle * (180 / Math.PI)) / 250) * 10);
		}

		for(daNote in notes){
			var spr = strumLineNotes[daNote.noteData + (daNote.mustPress ? 4 : 0)];
			var distance = Math.abs(Conductor.songPosition - daNote.strumTime);
			daNote.pitch = spr.pitch;
			daNote.z = Math.sin(daNote.pitch * (Math.PI / 180)) * distance;
		}*/
		...
		if (unspawnNotes[0] != null)
		{
			...
			{
				var dunceNote:Note3D = unspawnNotes[0];
		...
		if (generatedMusic)
		{
			...
			for(daNote in notes)
			{
				...
				if(daNote.mustPress) {
					strumX = playerStrums[daNote.noteData].x;
					strumY = playerStrums[daNote.noteData].y;
					strumZ = playerStrums[daNote.noteData].z;
					strumAngle = playerStrums[daNote.noteData].roll;
					//strumAlpha = playerStrums[daNote.noteData].alpha;
				} else {
					strumX = opponentStrums[daNote.noteData].x;
					strumY = opponentStrums[daNote.noteData].y;
					strumZ = opponentStrums[daNote.noteData].z;
					strumAngle = opponentStrums[daNote.noteData].roll;
					//strumAlpha = opponentStrums[daNote.noteData].alpha;
				}
					...
					daNote.strumTime <= Conductor.songPosition)
				{
					...
					notes.splice(notes.indexOf(daNote), 1);
					daNote.kill();
				}
				...
				if(daNote.mustPress) daNote.canBeHit = Math.abs(Conductor.songPosition - daNote.strumTime) <= Conductor.stepCrochet / 1.25;

				// WIP interpolation shit? Need to fix the pause issue
				// daNote.y = (strumLine.y - (songTime - daNote.strumTime) * (0.45 * PlayState.SONG.speed));

				var doKill:Bool = daNote.strumTime < Conductor.songPosition - (Conductor.stepCrochet * 1.5);
	...
	public inline function remove3D(){
		for(st in strumLineNotes)
			st.kill();
		for(nt in notes)
			nt.kill();

		Main.daScene.screen.cameras = [];
		remove(Main.daScene.screen);
		Main.daScene.kill();
		Main.daScene.render();
		Main.daScene.resetBitmapData();
	}
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				else
				{
					...
					trace(Paths.formatToSongPath(PlayState3D.storyPlaylist[0]) + difficulty);
					...
					PlayState3D.SONG = Song.loadFromJson(PlayState3D.storyPlaylist[0] + difficulty, PlayState3D.storyPlaylist[0]);
					FlxG.sound.music.stop();
					...
					if(winterHorrorlandNext) {
						new FlxTimer().start(1.5, function(tmr:FlxTimer) {
							...
							LoadingState.loadAndSwitchState(new PlayState3D());
						});
					} else {
						cancelFadeTween();
						//resetSpriteCache = true;
	...
	/*public function KillNotes() {
		while(notes.length > 0) {
			var daNote:Note3D = notes.members[0];
			daNote.active = false;
			daNote.visible = false;
	...
	private function popUpScore(note:Note3D = null):Void
	{
	...
	private function keyShit():Void
	{
		...
		if (!boyfriend.stunned && generatedMusic)
		{
			...
			{
				// hold note functions
				if (daNote.isSustainNote && controlHoldArray[daNote.noteData] && daNote.mustPress && daNote.strumTime <= Conductor.songPosition) {
					goodNoteHit(daNote);
				}
			}
			...
			if ((controlHoldArray.contains(true) || controlArray.contains(true)) && !endingSong) {
				...
				if (controlArray.contains(true)) {
					for (i in 0...controlArray.length) {
						...
						var pressNotes:Array<Note3D> = [];
						var notesDatas:Array<Int> = [];
						var notesStopped:Bool = false;

						var sortedNotesList:Array<Note3D> = [];
						for(daNote in notes)
						{
							...
							for (epicNote in sortedNotesList)
							{
								for (doubleNote in pressNotes) {
									if (Math.abs(doubleNote.strumTime - epicNote.strumTime) < 10) {
										notes.splice(notes.indexOf(doubleNote), 1);
										doubleNote.kill();
									} else
										notesStopped = true;
								}
		...
		for(strum in playerStrums){
			...
			if(controlReleaseArray[strum.ID]) 
				if(!strum.isGray) {
					strum.sPlane.material = [lsTex, dsTex, usTex, rsTex][strum.ID];
					strum.isGray = true;
				}
		}
	}
	...
	function noteMiss(daNote:Note3D):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		//Dupe note remove
		for(note in notes)
	...
	function goodNoteHit(note:Note3D):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(cpuControlled) {
				...
				//});
				for(strum in playerStrums){
					if(note.noteData == strum.ID && strum.isGray){
						strum.sPlane.material = [lcTex, dcTex, ucTex, rcTex][strum.ID];
						strum.isGray = false;
					}
				}
			}
	...
	/*public function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		...
		if(PlayState3D.SONG.splashSkin != null && PlayState3D.SONG.splashSkin.length > 0) skin = PlayState3D.SONG.splashSkin;
	...
	override function beatHit()
	{
		...
		if (generatedMusic && PlayState3D.SONG.notes[Std.int(curStep / 16)] != null && !endingSong && !isCameraOnForcedPos)
		{
